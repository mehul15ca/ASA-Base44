      ...
      <div className="p-8 bg-gradient-to-br from-[#0A1F0A] via-[#0D2818] to-[#0A1F0A] min-h-full">
              ...
              >
                <Card className="bg-gradient-to-br from-[#1A4D2E] to-[#0D2818] border-[#2D6A4F]/50 p-6">
                  <div className="flex items-start justify-between mb-4">
                    ...
                    <Badge className={coach.status === 'Active' ? 'bg-green-500/20 text-green-400' : 'bg-gray-500/20 text-gray-400'}>
                      {coach.status}
                    </Badge>
                  </div>
                  
                  <h3 className="text-xl font-bold text-white mb-1">{coach.name}</h3>
                  <p className="text-[#D4AF37] text-sm mb-3">{coach.sport}</p>
                  ...
                  <div className="mb-4">
                    <p className="text-gray-400 text-xs mb-2">Batches Covered:</p>
                    <div className="flex flex-wrap gap-1">
                      {coach.batches.length > 0 ? (
                        coach.batches.map((batch, i) => (
                          <Badge key={i} variant="outline" className="border-[#40916C] text-[#40916C] text-xs">
                            {batch}
                          </Badge>
                        ))
                      ) : (
                        <span className="text-gray-500 text-xs">No batches assigned</span>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="flex gap-2">
                    <Button
                      ...
                      className="flex-1 border-[#40916C] text-[#40916C] hover:bg-[#40916C]/10"
                    >
                      View Salary
                    </Button>
                    <Button
            ...
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead className="bg-[#0D2818] border-b border-[#2D6A4F]/50">
                  <tr>
                    <th className="px-6 py-4 text-left text-sm font-semibold text-gray-300">Coach Name</th>
                    <th className="px-6 py-4 text-left text-sm font-semibold text-gray-300">Total Earned</th>
                ...
                <tbody>
                  {filteredCoaches.map((coach, index) => (
                    ...
                    >
                      ...
                      <td className="px-6 py-4 text-sm text-[#D4AF37] font-semibold">${coach.totalEarned.toLocaleString()}</td>
                      <td className="px-6 py-4 text-sm text-green-400">${coach.paid.toLocaleString()}</td>
                      <td className="px-6 py-4 text-sm text-yellow-400">${coach.pending.toLocaleString()}</td>
                      <td className="px-6 py-4">
                        <Badge className={coach.pending === 0 ? 'bg-green-500/20 text-green-400' : 'bg-yellow-500/20 text-yellow-400'}>
        ...
        <Dialog open={showSalaryDetailsModal} onOpenChange={setShowSalaryDetailsModal}>
          <DialogContent className="bg-[#0D2818] border-[#2D6A4F] max-w-2xl">
            <DialogHeader>
              <DialogTitle className="text-white text-xl">Salary Details</DialogTitle>
            </DialogHeader>
            
            <div className="space-y-6">
              <div>
                <p className="text-gray-400 text-sm">Coach</p>
                <p className="text-white font-semibold text-lg">{selectedCoach?.name}</p>
                <p className="text-gray-400 text-sm">{selectedCoach?.email}</p>
              </div>

              <div className="grid grid-cols-3 gap-4">
                <Card className="bg-[#0A1F0A] border-[#2D6A4F]/50 p-4">
                  <p className="text-gray-400 text-sm mb-1">Total Earned</p>
                  <p className="text-[#D4AF37] font-bold text-xl">${selectedCoach?.totalEarned.toLocaleString()}</p>
                </Card>
                <Card className="bg-[#0A1F0A] border-[#2D6A4F]/50 p-4">
                  <p className="text-gray-400 text-sm mb-1">Total Paid</p>
                  <p className="text-green-400 font-bold text-xl">${selectedCoach?.paid.toLocaleString()}</p>
                </Card>
                <Card className="bg-[#0A1F0A] border-[#2D6A4F]/50 p-4">
                  <p className="text-gray-400 text-sm mb-1">Pending</p>
                  <p className="text-yellow-400 font-bold text-xl">${selectedCoach?.pending.toLocaleString()}</p>
                </Card>
              </div>
                  ...
                  {salaryHistory.map((payment, i) => (
                    <div key={i} className="flex justify-between items-center py-3 border-b border-[#2D6A4F]/30 last:border-0">
                      ...
                      <div className="flex items-center gap-3">
                        <p className="text-[#D4AF37] font-bold">${payment.amount.toLocaleString()}</p>
