  ...
  return (
    <AdminLayout>
      <div className="p-4 md:p-8 bg-gradient-to-br from-[#0A1F0A] via-[#0D2818] to-[#0A1F0A] min-h-full">
        ...
        >
          <h1 className="text-xl md:text-3xl font-bold text-white mb-1 md:mb-2">Batch Management</h1>
          <p className="text-xs md:text-sm text-gray-400">Manage batches, schedules, and student groups</p>
        </motion.div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-3 md:gap-6 mb-4 md:mb-8">
          {stats.map((stat, index) => (
            <motion.div
              ...
              className="bg-gradient-to-br from-[#1A4D2E] to-[#0D2818] border border-[#2D6A4F]/50 rounded-lg md:rounded-xl p-4 md:p-6"
            >
        ...
        <motion.div
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          className="bg-gradient-to-br from-[#1A4D2E] to-[#0D2818] border border-[#2D6A4F]/50 rounded-lg md:rounded-xl p-4 md:p-6 mb-4 md:mb-6"
        >
          <div className="flex flex-col md:flex-row flex-wrap gap-2 md:gap-4">
            <div className="relative flex-1 min-w-[150px] md:min-w-[200px]">
              ...
              <Input
                ...
                className="pl-10 bg-[#0A1F0A] border-[#2D6A4F] text-white text-xs md:text-sm h-8 md:h-10"
              />
            </div>
            <Select value={sportFilter} onValueChange={setSportFilter}>
              <SelectTrigger className="w-full md:w-48 bg-[#0A1F0A] border-[#2D6A4F] text-white text-xs md:text-sm h-8 md:h-10">
                <SelectValue />
              </SelectTrigger>
            ...
            <Button
              ...
              className="bg-gradient-to-r from-[#D4AF37] to-[#F4D03F] text-[#0A1F0A] w-full md:w-auto text-xs md:text-sm h-8 md:h-10"
            >
        ...
        >
          <div className="overflow-x-auto">
            <table className="w-full text-xs md:text-sm">
              <thead className="bg-[#0D2818] border-b border-[#2D6A4F]/50">
                <tr>
                  <th className="px-3 md:px-6 py-2 md:py-4 text-left font-semibold text-gray-300">Batch Name</th>
                  <th className="px-3 md:px-6 py-2 md:py-4 text-left font-semibold text-gray-300 hidden sm:table-cell">Sport</th>
                  <th className="px-3 md:px-6 py-2 md:py-4 text-left font-semibold text-gray-300 hidden md:table-cell">Coach</th>
                  <th className="px-3 md:px-6 py-2 md:py-4 text-left font-semibold text-gray-300 hidden lg:table-cell">Ground</th>
                  <th className="px-3 md:px-6 py-2 md:py-4 text-left font-semibold text-gray-300">Students</th>
                  <th className="px-3 md:px-6 py-2 md:py-4 text-left font-semibold text-gray-300 hidden lg:table-cell">Schedule</th>
                  <th className="px-3 md:px-6 py-2 md:py-4 text-left font-semibold text-gray-300">Status</th>
                  <th className="px-3 md:px-6 py-2 md:py-4 text-left font-semibold text-gray-300">Actions</th>
                </tr>
              </thead>
              <tbody>
                {paginatedBatches.map((batch, index) => (
                  <motion.tr
                   ...
                   >
                     <td className="px-3 md:px-6 py-2 md:py-4 text-white font-medium">{batch.name}</td>
                     <td className="px-3 md:px-6 py-2 md:py-4 text-gray-300 hidden sm:table-cell">{batch.sport}</td>
                     <td className="px-3 md:px-6 py-2 md:py-4 text-gray-300 hidden md:table-cell">{batch.coach}</td>
                     <td className="px-3 md:px-6 py-2 md:py-4 text-gray-300 hidden lg:table-cell">{batch.ground}</td>
                     <td className="px-3 md:px-6 py-2 md:py-4 text-[#D4AF37] font-semibold">{batch.students}</td>
                     <td className="px-3 md:px-6 py-2 md:py-4 text-gray-300 hidden lg:table-cell">{batch.schedule}</td>
                     <td className="px-3 md:px-6 py-2 md:py-4">
                       <Badge className={`${batch.status === 'Active' ? 'bg-green-500/20 text-green-400' : 'bg-gray-500/20 text-gray-400'} text-xs`}>
                         {batch.status}
                       </Badge>
                     </td>
                     <td className="px-3 md:px-6 py-2 md:py-4">
                       <div className="flex gap-1 md:gap-2">
                         <Button
                           ...
                           className="border-[#40916C] text-[#40916C] hover:bg-[#40916C]/10 h-7 md:h-9"
                         >
                         ...
                         <Button
                           size="sm"
                           variant="outline"
                           className="border-red-500/50 text-red-400 hover:bg-red-500/10 h-7 md:h-9"
                         >
          ...
          {/* Pagination */}
          <div className="px-3 md:px-6 py-2 md:py-4 border-t border-[#2D6A4F]/50 flex flex-col md:flex-row justify-between items-center gap-2 md:gap-0">
            <p className="text-xs md:text-sm text-gray-400">
              Showing {startIndex + 1} to {Math.min(endIndex, filteredBatches.length)} of {filteredBatches.length}
            </p>
            <div className="flex gap-1 md:gap-2 w-full md:w-auto">
              <Button
                ...
                className="border-[#40916C] text-gray-300 flex-1 md:flex-none text-xs md:text-sm h-7 md:h-9"
              >
          ...
          <DialogContent className="bg-[#0D2818] border-[#2D6A4F] max-w-2xl max-h-[90vh] overflow-y-auto">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-4">
                <div>
                   ...
                   <Select defaultValue={editingBatch?.sport?.toLowerCase()}>
                     <SelectTrigger className="bg-[#0A1F0A] border-[#2D6A4F] text-white text-xs md:text-sm h-8 md:h-10">
            ...
            <DialogFooter className="flex-col-reverse md:flex-row gap-2">
              <Button
                ...
                className="border-[#40916C] text-gray-300 text-xs md:text-sm h-8 md:h-10 w-full md:w-auto"
              >
                Cancel
              </Button>
              <Button
                ...
                className="bg-gradient-to-r from-[#D4AF37] to-[#F4D03F] text-[#0A1F0A] text-xs md:text-sm h-8 md:h-10 w-full md:w-auto"
              >
                Save Batch
              </Button>
            </DialogFooter>
