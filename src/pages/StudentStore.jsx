      ...
      <div className="p-8">
                ...
                >
                  <Card className="bg-gradient-to-br from-[#1A4D2E] to-[#0D2818] border-[#2D6A4F]/50 overflow-hidden">
                    ...
                    <div className="p-4">
                      ...
                      <Badge className="bg-[#D4AF37]/20 text-[#D4AF37] mb-3">{product.category}</Badge>
                      <div className="flex justify-between items-center mb-4">
                        <span className="text-2xl font-bold text-white">₹{product.price.toLocaleString()}</span>
                        <Badge className={product.stock === 'In Stock' ? 'bg-green-500/20 text-green-400' : 'bg-red-500/20 text-red-400'}>
                          {product.stock}
                        </Badge>
                      </div>
                      <Button
        ...
        <Dialog open={!!selectedProduct} onOpenChange={() => {
          ...
          <DialogContent className="bg-[#0D2818] border-[#2D6A4F]">
            <DialogHeader>
              <DialogTitle className="text-white">Order Product</DialogTitle>
            </DialogHeader>
            <div className="space-y-4">
              <div className="flex gap-4">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-[#D4AF37] text-xl font-bold mt-2">₹{selectedProduct?.price.toLocaleString()}</p>
                </div>
              </div>
              <div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <Input
                    ...
                    className="w-20 text-center bg-[#0A1F0A] border-[#2D6A4F] text-white"
                  />
                  <Button
                    size="sm"
                    variant="outline"
        ...
        <Dialog open={!!selectedOrder} onOpenChange={() => setSelectedOrder(null)}>
          <DialogContent className="bg-[#0D2818] border-[#2D6A4F]">
            ...
            <div className="space-y-4">
              <div className="bg-[#0A1F0A]/50 rounded-lg p-4">
                <h3 className="text-[#D4AF37] text-xl font-bold mb-2">{selectedOrder?.id}</h3>
              ...
              <div className="flex justify-between bg-[#1A4D2E] rounded-lg p-4">
                ...
                <span className="text-[#D4AF37] text-xl font-bold">₹{selectedOrder?.total.toLocaleString()}</span>
              </div>
              <div>
                <p className="text-gray-400 text-sm mb-2">Status</p>
                <div className="flex items-center gap-2">
                  {selectedOrder?.status === 'Delivered' && <CheckCircle className="w-5 h-5 text-green-400" />}
                  {selectedOrder?.status === 'In Transit' && <Truck className="w-5 h-5 text-blue-400" />}
                  {selectedOrder?.status === 'Pending' && <Package className="w-5 h-5 text-yellow-400" />}
                  <Badge className={
